...
describe('Suggestion Box', () => {
    ...
    beforeEach(() => { suggestionBoxRepo.clear() })
  ...
  describe('/GET suggestion boxes', () => {
    ...
    it('should GET all of the suggestion boxes when there is one', (done) => {
        const box = suggestionBoxRepo.createBox({name: "New menu items for cafeteria"})
        chai.request(server)
            .get('/suggestionbox')
            .end((err, res) => {
                res.should.have.status(200)
        ...
        it('should GET an existing suggestion box by owner hash', (done) => {
            chai.request(server)
                ...
                .end((err , res) => {
                    ...
                    res.body.should.have.property('hash_submitter').eql(box.hash_submitter)
